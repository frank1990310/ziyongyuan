...
class Spider(SpiderBase):
    ...
    def _pic_url(self, pic):
        """封面走本地代理，解决 CDN Content-Type / 防盗链导致不显示"""
        if not pic:
            return ""
        if pic.startswith("//"):
            pic = "https:" + pic
        proxy = ""
        try:
            proxy = self.getProxyUrl() or ""
        except Exception:
            proxy = ""
        if proxy and pic.startswith("http"):
            # 代理拉图并改成 image/jpeg
            b64 = base64.urlsafe_b64encode(pic.encode("utf-8")).decode("utf-8")
            sep = "&" if "?" in proxy else "?"
            return "%s%sdo=pic&u=%s" % (proxy, sep, b64)
        # 无代理时直接返回，并带 Referer 后缀（部分壳支持）
        return pic
    ...
    def detailContent(self, ids):
        ...
        pic = self._pic_url(info.get("cover_url") or info.get("thumb_url") or "")
